...
df_va_stts<-rbind(read.csv("../experiment/data/0723/states.csv"),
                    read.csv("../experiment/data/0726/states.csv"),
                    read.csv("../experiment/data/0728/states.csv"),
                    read.csv("../experiment/data/0729/states.csv"))
...
df <- read_csv("../experiment/data/qualtrics_text.csv")
df <- df[-c(1,2),]
df<-df%>%
...
df_temp %>%
  ...
  ggplot(aes(sd, group=question, color=question)) + 
  geom_density(adjust=1.5, alpha=.2) +
  geom_vline(aes(xintercept=median, color=question),
             linetype="dashed")+
  ...
  scale_colour_manual(values = color_blind_friendly) -> plt_sd
```
...
df_divisiveness%>%
  ...
  ggplot(aes(divisiveness, group=question, color=question)) + 
  geom_density(adjust=1.5, alpha=.2)+xlab("divisiveness") +
  geom_vline(aes(xintercept=median, color=question),linetype="dashed") +
  ...
  scale_colour_manual(values = color_blind_friendly) ->plt_divisiveness
```
...
df_temp%>% 
  ...
  geom_point(aes(group = question, colour=factor(question)), size=3, alpha=.5)+  
