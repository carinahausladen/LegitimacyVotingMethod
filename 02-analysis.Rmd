...
color_blind_friendly <- c("#FFB000", "#D35FB7", "#005AB5", "#DC3220", "#009E73","#785EF0")
...
df_va_stts_lng%>% 
 ...
 # select("id_q", "vote_mechanism","option","score_scld") %>%
  filter(id_q=="13586_Q3") %>%
  
  mutate(score_scld = ifelse(option == 'o2', score_scld - 0.01,
                        ifelse(option == 'o3', score_scld - 0.015, 
                               ifelse(option == 'o1', score_scld - 0.02, 
                                      ifelse(option == 'o5', score_scld - 0.025, 
                      score_scld))))) %>%
  
  ggplot(.,aes(x=vote_mechanism, y=score_scld, group=option, color=option)) +
  geom_point(size=1)+
  geom_line(aes(group = option), alpha=0.8)+  
...
df_temp%>% 
  ...
  ggplot(., aes(`mv, cav, sv, mbc`, n_group)) + 
  geom_segment(aes(x=`mv, cav, sv, mbc`, xend=`mv, cav, sv, mbc`, 
                    y=0, yend=n_overall), color="grey") +
  geom_point(aes(group = question, color=question, shape=question), size=3, alpha=.8)+  
  ...
  theme(legend.position = "bottom",
        text=element_text(size=12),
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) ->plt_temp
 # scale_color_manual(legend_title,values = color_blind_friendly) 

#  guides(fill = guide_legend(override.aes = list(color = color_blind_friendly,shape = color_blind_friendly)))
